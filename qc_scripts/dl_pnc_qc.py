...
qsiprep_results = [ana for ana in analyses if
                   ana.label.startswith("qsiprep2mm")]
...
failed_analyses = []
out_dir = "/Users/mcieslak/projects/writing/qsiprep_paper/qc_scripts/qsiprep_pnc_tsvs"
...
pnc_files = glob(out_dir + "/*ImageQC*csv")

dfs = [pd.read_csv(fname) for fname in pnc_files]
pnc_df = pd.concat(dfs, axis=0)
pnc_df.to_csv("pnc_results.csv", index=False)
