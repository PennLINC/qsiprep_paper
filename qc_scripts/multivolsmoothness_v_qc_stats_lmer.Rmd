...
abcd.test <- lmer(qc ~ fwhm.cen + pipeline + (1 | subject), abcd)
summary(abcd.test)
...
hcp.test <- lmer(qc ~ fwhm.cen + pipeline + (1 | subject), hcp)
summary(hcp.test)
...
pnc.test <- lmer(qc ~ fwhm.cen + pipeline + (1 | subject), pnc)
summary(pnc.test)
...
noddi.test <- lmer(qc ~ fwhm.cen + pipeline + (1 | subject), noddi)
summary(noddi.test)
...
hbn.test <- lmer(qc ~ fwhm + pipeline + (1 | subject), hbn)
summary(hbn.test)
...
fwhm.models <- rbind(
  ...
  tidy(pnc.fwhm.model) %>% mutate(scheme="DTI 64")
    ...
    mutate_each(funs(round(., 3)), -term, -scheme, -effect, -group)

fwhm.mmodels <- subset(fwhm.models, term=="pipelineQSIPrep")[,c("scheme", "estimate", "std.error", "statistic", "p.value")]
